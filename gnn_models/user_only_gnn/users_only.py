...
user_only_embedding_path = 'embeddings/emb_users_only.npy'
user_text_embedding_path = 'covtwit/embeddings/textv2/embeddings_550k.npy'
full_user_text_embedding_path = 'covtwit/embeddings/textv2/embeddings_850k.npy'
...
def import_users_only(edge_path, attr_path, sm_edge_path, label_path):
    ...
    #labeldf[labeldf['url'].isin(other_sites['url'].tolist())]
    labeldf = pd.concat([labeldf, other_sites])
    
    el_sm.columns = ['domain_to', 'domain_from']
    #labeldf = labeldf[labeldf['url'].isin(el_sm['domain_to'])]
...
def to_pt_n2v_tensors(ulabs, el):
    ...
    edge_list = torch.tensor([el['source'].values, el['target'].values]).long()
...
def node_masker_n2v(edge_path, attr_path, embedding_path, sm_edge_path, label_path, user_feats = True):
    ulabs, el, url_mapper = import_users_only(edge_path, attr_path, sm_edge_path, label_path)
    data = to_pt_n2v_tensors(ulabs, el)
    ...
    if user_feats:
        embeddings = np.load(full_user_text_embedding_path)
        embeddings = embeddings[user.id.values]
    else:
...
def node_masker_n2v_curriculum(edge_path, attr_path, embedding_path, sm_edge_path, label_path, binary_labels = True, user_feats = True):
    ulabs, el, url_mapper = import_users_only(edge_path, attr_path, sm_edge_path, label_path)
    ...
    #labeled_nodes[labeled_nodes.id.isin(test_mask.numpy())].label.value_counts()
    ...
    if user_feats:
        embeddings = np.load(full_user_text_embedding_path)
        embeddings = torch.FloatTensor(embeddings[user.id.values])
    else:
        embeddings = np.load(embedding_path)
...
def partial_f1_graph(edge_path, attr_path, embedding_path, sm_edge_path, label_path):
    ...
    hetdat['websites'].y = torch.tensor(urls.label.values).long()
    hetdat['websites'].unlabeled_mask = unlabeled_mask
    hetdat['websites'].labeled_mask = labeled_mask
    #hetdat['websites'].train_mask = train_mask
    #hetdat['websites'].val_mask = val_mask
    #hetdat['websites'].test_mask = test_mask
    hetdat['users'].x = torch.FloatTensor(user_x)
